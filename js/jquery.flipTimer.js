(function($) {
  ...
  var flipTimer = function(element, options) {
    ...
    // store the date/time when initialized
    ...
    renderDigits: function(subject, value) {
      ...
      if ($(subject).find('.digit').length == 0) {
        ...
        if (_this.days < 0 && _this.hours < 0 && _this.minutes < 0 && _this.seconds < 0) {
          ...
          number_array = [String(value)]; // split all digits
        ...
        $(subject).find('.digit-set').each(function(el) {
          ...
          // console.log($(this).parent().closest('div')); // Get its parent
          if ($(this).parent().closest('div').hasClass("hours")) {
            max = (el == 0) ? maxDigit : 23;
          }
          else if ($(this).parent().closest('div').hasClass("minutes") || $(this).parent().closest('div').hasClass("seconds")) {
            max = (el == 0) ? maxDigit : 59;
          }
          else if ($(this).parent().closest('div').hasClass("days")) {
            max = (el == 0) ? maxDigit : 999;
          }
